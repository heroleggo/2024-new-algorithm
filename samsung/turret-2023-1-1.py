...
def bfs(x, y, visited, parents):
    ...
    while queue:
        ...
        for i in range(4):
            ...
            if turrets[nx][ny] != 0 and visited[nx][ny] > visited[cx][cy] + 1:
...
def get_attack_path(attack, target):
    ...
    bfs(attack[0], attack[1], visited, parent)
...
def attack_cannon(target_pos, attack_pos):
    ...
    for idx, delta in enumerate(cannon_range):
        ...
        if next_turret != 0 and (nx != attack_pos[0] or ny != attack_pos[1]):
            if idx == 0:
